...
export function PartFamily({variants,state,parts,onSelect,onDetail}:{variants:Part[];state:BuildState;parts:Part[];onSelect:(p:Part)=>void;onDetail:(p:Part)=>void}){
 const [choice,setChoice]=useState<string>();
 const p=variants.find(v=>v.id===choice)??variants.find(v=>v.id===state.picks[v.category])??variants[0];
 ...
 return <article className={`part-card ${active?'selected':''}`}>
  <div className="part-card-top"><span className="part-brand">{p.brand}</span><span className="part-type">{variants.length===1?'1 option':`${variants.length} options`}</span></div>
  ...
  {variants.length>1?<Select value={p.id} onValueChange={setChoice}><SelectTrigger className="variant-select" aria-label={`${p.brand} ${p.name} variant`}><SelectValue/></SelectTrigger><SelectContent>{variants.map(v=><SelectItem key={v.id} value={v.id}>{v.variant} · {money(v.priceCents)} / {v.category==='tires'||v.category==='wheels'?'each':v.category==='armor'?'pair':'kit'}</SelectItem>)}</SelectContent></Select>:<p className="single-variant">{p.variant}</p>}
  <button className="details-link" onClick={()=>onDetail(p)}>Specs & fitment notes<ArrowUpRight size={13}/></button>
  {conflicts.length>0&&<p className="option-conflict">With this option: {conflicts.length} build conflict{conflicts.length===1?'':'s'}. Review before buying.</p>}
  <div className="part-bottom"><div><strong>{money(p.priceCents*qty)}</strong><span> / {qty>1?`${qty} ${p.category}`:p.category==='armor'?'pair':'kit'}</span><small>{qty>1?`${money(p.priceCents)} each · `:''}{p.customPrice?'Your price note':'Source snapshot'}</small></div><Button size="sm" variant={active?'secondary':'outline'} onClick={()=>onSelect(p)}>{active?<><Check size={15}/>Added</>:<><Plus size={15}/>{current?'Replace':'Add'}</>}</Button></div>
