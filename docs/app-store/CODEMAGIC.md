...
- Next: verify/register this app's Apple identifier, then finish its App Store Connect record and provisioning profile before using the TestFlight workflow on current `main`. No repeat unsigned build is needed just to establish the baseline already confirmed above. The compile-check run cannot be installed through TestFlight.
...
- In Apple Developer → Certificates, Identifiers & Profiles → Identifiers, first check for `com.johnprodromidis.jeepbuildlab` in that Apple team. If absent, choose **+ → App IDs → App**, description **Jeep Build Lab**, and an **Explicit** Bundle ID of `com.johnprodromidis.jeepbuildlab`, then Continue → Register. The current app requires no additional optional capabilities. [Apple registration steps, checked September 10](https://developer.apple.com/help/account/identifiers/register-an-app-id/)
